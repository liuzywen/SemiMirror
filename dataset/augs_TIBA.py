...
class ToTensorAndNormalize(object):
    ...
    def __call__(self, in_image, in_label):
        ...
        label = (torch.from_numpy(np.array(in_label, dtype=np.int32))/255).long()
        # label = Image.fromarray(np.int32(in_label))
        # label = self.to_tensor(in_label)
    ...
    def __call__(self, in_image, in_label):
        ...
        if isinstance(self.base_size, int):
            ...
            if self.scale:
                ...
                # print('xxx',scale, self.base_size)
                # print('xxx', oh, ow)
            else:
